...
from config import gp_retain_ratio
...
def _parallel_evolve(n_programs, parents, X, y, sample_weight, seeds, params):
    ...
    if parents is None:
        ...
        count=len(parents)
    fitness = [parents[p].fitness_ for p in range(count)]
    if metric.greater_is_better:
        retain_index=list(pd.Series(fitness).sort_values(ascending=False).index)[:int(gp_retain_ratio*count)]
        for index in retain_index:
            programs.append(parents[index])
    else:
        retain_index=list(pd.Series(fitness).sort_values(ascending=True).index)[:int(gp_retain_ratio*count)]
        for index in retain_index:
            programs.append(parents[index])

    for i in range(n_programs-int(gp_retain_ratio*count)):
        ...
        #indices, not_indices = program.get_all_indices(n_samples,
                                                       #max_samples,
                                                       #random_state)
        # train / test set split, use first two year as train set,
        # the last following year as test set.                                                       
        _, dateRange, _ = X.shape
        splitPoint = 2*int(dateRange) // 3
        
        not_indices = np.arange(splitPoint, dateRange)
        indices = np.arange(0, splitPoint)     
...
class BaseSymbolic(BaseEstimator, metaclass=ABCMeta):
    ...
    def fit(self, X, y, sample_weight=None,us=None):
        ...
        if isinstance(self, ClassifierMixin):
            ...
            X, y = check_X_y(X, y, y_numeric=True, allow_nd=True, multi_output=True)
        if sample_weight is not None:
            sample_weight = check_array(sample_weight, ensure_2d=False)
        _, _, self.n_features_ = X.shape
        ...
        self._paras = {}
        for function in self._function_set:
            if function.para is None:
                para = None
            else:
                para = [str(p) for p in function.para]
                para = ''.join(para)
            self._paras[para] = self._paras.get(para, [])
            self._paras[para].append(function)
        ...
        for gen in range(prior_generations, self.generations):
            ...
            if (gen == 0)&(us is None):
                ...
                for alpha in us:
                    ...
                    for i in alpha:
                        if type(i)==str:
                            us_ulti.append(_function_map[i])
                        else:
                            us_ulti.append(i)
                    program = _Program(function_set=function_set,
                                       arities=arities,
